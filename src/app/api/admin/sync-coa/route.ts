...
export async function POST() {
  ...
  // Hapus semua COA lama (transaksi sudah kosong setelah reset, aman)
  await prisma.coaAccount.deleteMany({});

  // Insert ulang dari master list
  for (let i = 0; i < COA_MASTER.length; i++) {
    const c = COA_MASTER[i];
    await prisma.coaAccount.create({
      data: {
        code: c.code,
        name: c.name,
        kategori: c.kategori,
        reportType: deriveReportType(c.kategori, c.name),
        urutan: i,
      },
    });
  }

  return NextResponse.json({ ok: true, message: `${COA_MASTER.length} akun COA berhasil di-sync dari master list.` });
}
